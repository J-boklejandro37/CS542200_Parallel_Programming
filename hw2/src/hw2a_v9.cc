...
// Making the chunk size larger than one row
// chunk size 1 rows -> 92.77 s
// chunk size 2 rows -> 92.93 s
...
struct Chunk
{
    ...
    int row_span;
    Chunk(){}
    Chunk(int r, int rs) : row(r), row_span(rs) {}
};
...
public:
    TaskQueue(int height) : current_chunk(0)
    {
        pthread_mutex_init(&mutex, nullptr);
        // Create tasks for each row
        chunks.resize((height + CHUNK_SIZE - 1) / CHUNK_SIZE);
        for (int row = 0; row < height; row += CHUNK_SIZE)
        {
            int row_span = std::min(CHUNK_SIZE, height - row);
            chunks.emplace_back(row, row_span);
        }
    }
...
private:
    ...
    void compute(int thread_id)
    {
        ...
        while (true)
        {
            Chunk chunk = task_queue->getNextChunk();
            if (chunk.row == -1) break; // No more rows to process

            int row = chunk.row;
            int row_span = chunk.row_span;

            for (int j = row; j < row + row_span; ++j)
            {
