// dynamic load balancing, vectorization
// chunk size 10 -> 292.00 s
// chunk size 20 -> 293.95 s
// chunk size 40 -> 293.95 s
// chunk size 80 -> 298.46 s
// chunk size 120 -> 300.09 s
...
int main(int argc, char** argv) {
    ...
    {   // Initialize MPI
        ...
        // Need process >= 2 for master-slave gerenation
...
void masterProcess(int num_processes, const std::string& filename, double left, double right, double lower, double upper, int width, int height, int iters)
{
    const int CHUNK_SIZE = 120; // num_rows for work unit
    ...
    while (!work_queue.empty() || finished_workers < num_processes - 1)
    {
        ...
        switch (status.MPI_TAG)
        {
            ...
            case RESULT:
            {
                ...
                // Store onto image
                mandelbrot.storeRows(result.start_row, result.data, result.num_rows);
...
void workerProcess(int rank, double left, double right, double lower, double upper, int width, int height, int iters)
{
    ...
    while (true)
    {
        ...
        std::vector<int> result = mandelbrot.computeRows(unit.start_row, unit.num_rows);
