// float_sort(omp) + iteration size + check before merge + half merge(less comparison) + Isend/Irecv (Time: 103.53s)
#include <cstdio>
#include <cstdlib>
#include <iostream>
#include <mpi.h>
...
int main(int argc, char* argv[])
{
    ...
    {   /*------------------------------------------- even-odd-even-odd sort -------------------------------------------*/
        ...
        {   // loads just one data for comparison
            ...
            MPI_Irecv(&right_arr[0], 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &r_req);
            ...
            {   // loads the whole data
                MPI_Isend(&self_arr[0], self_count - 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &s_req);
        ...
        {
            MPI_Isend(&self_arr[0], 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &s_req);
            ...
            {   // loads the whole data
                ...
                MPI_Irecv(&left_arr[0], left_count - 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &r_req);
                MPI_Wait(&r_req, MPI_STATUS_IGNORE);
                MPI_Wait(&s_req, MPI_STATUS_IGNORE);
