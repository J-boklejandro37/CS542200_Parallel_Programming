// partial Allreduce + normal merge + Send/Recv (Time: 282.07s, slower than Ver.3)
#include <cstdio>
#include <cstdlib>
#include <iostream>
#include <mpi.h>
...
int main(int argc, char* argv[])
{
    ...
    int iteration = 1;
    int threshold = 10;
    while (!sorted && iteration <= size)
    {
        // if (rank == 0) cout << "iteration " << iteration << endl;
        /*------------------------------------------- even sort -------------------------------------------*/
        if (rank % 2 == 0 && rank < size - 1) // first half
        {
            MPI_Send(self_arr.data(), self_count, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD);
        ...
        if (iteration % threshold == 0) MPI_Allreduce(MPI_IN_PLACE, &sorted, 1, MPI_C_BOOL, MPI_LAND, MPI_COMM_WORLD);
        else sorted = false;
