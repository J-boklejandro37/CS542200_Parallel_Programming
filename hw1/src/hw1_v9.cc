// iteration size + check before merge + half merge + Sendrecv (Time: 127.8s)
#include <cstdio>
#include <cstdlib>
#include <iostream>
#include <mpi.h>
...
void merge_and_keep(vector<float>& arr1, vector<float>& arr2, vector<float>& buff, bool is_left)
{
    ...
    else
    {
        ...
        while (idx1 >= 0 && idx2 >= 0 && buffidx >= 0)
        {
            if (arr2[idx2] >= arr1[idx1]) buff[buffidx--] = arr2[idx2--];
            else buff[buffidx--] = arr1[idx1--];
        }
        while (idx1 >= 0 && buffidx >= 0) buff[buffidx--] = arr1[idx1--];
        while (idx2 >= 0 && buffidx >= 0) buff[buffidx--] = arr2[idx2--];
        swap(arr2, buff);
    }
}
