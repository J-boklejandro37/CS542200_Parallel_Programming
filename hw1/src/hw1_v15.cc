// no vector + float_sort(omp) + iteration size + check before merge + half merge(less comparison) + Sendrecv (Time: 97.66ss)
#include <cstdio>
#include <cstdlib>
#include <iostream>
#include <mpi.h>
    ...
    {   /*------------------------------------------- even-odd-even-odd sort -------------------------------------------*/
        ...
        {   // loads just one data for comparison
            MPI_Sendrecv(self_arr + self_count - 1, 1, MPI_FLOAT, rank + 1, 0, 
                         right_arr, 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        else if (!is_left && rank != 0 && self_count != 0 && left_count != 0) // right part
        {
            ...
            {   // loads the whole data
                MPI_Sendrecv(self_arr + 1, self_count - 1, MPI_FLOAT, rank - 1, 0,
                            left_arr, left_count - 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
