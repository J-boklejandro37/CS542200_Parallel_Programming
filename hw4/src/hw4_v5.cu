// testcase t20
// -memory_coalesce_with_transpose (still trying)
    ...
    const int Bc, const int Br, const int Tc, const int Tr, const float softmax_scale)
{
    ...
    int q_tile_size = Br * (d + PAD);
    int k_tile_size = Bc * (d + PAD);
    int v_tile_size = Bc * (d + PAD);
    ...
    float* sh_iter = Qi + local_y * (d + PAD);
    ...
    for (int tile_idx_x = 0; tile_idx_x < Tc; tile_idx_x++)
    {
        ...
        for (int y = local_y; y < Bc; y += Br) {
            ...
            if (k_global_y < N) {
                float* sh_iter_k = Ki + y * (d + PAD);
                float* sh_iter_v = Vi + y * (d + PAD);

                #pragma unroll
                for (int x = 0; x < d; x++) {
                    // Column-major access for K and V
                    *(sh_iter_k + x) = d_K[batch_offset + x * N + k_global_y];
                    *(sh_iter_v + x) = d_V[batch_offset + x * N + k_global_y];
                }
            }
        }
        ...
        for (int j = 0; j < tile_width; j++) {
            ...
            #pragma unroll
            for (int k = 0; k < d; k++) {
                sij += Qi[local_y * (d + PAD) + k] * Ki[j * (d + PAD) + k];
            }
        ...
        for (int x = 0; x < d; x++) {
            ...
            #pragma unroll
            for (int j = 0; j < tile_width; j++) {
                pv += Sij[local_y * Bc + j] * Vi[j * (d + PAD) + x];
            }

            // Access O in column-major order
            int o_idx = batch_offset + x * N + global_y;
            d_O[o_idx] = (1 / li_new) * 
                ((li * __expf(mi - mi_new) * d_O[o_idx]) +
...
__global__
void matrix3D_inner_transpose(float* input, float* output,
                                     int depth, int height, int width)
{
    __shared__ float tile[32][32];
    
    // Calculate indices for depth slice
    int d = blockIdx.z;
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    
    // Input index with depth stride
    int slice_size = width * height;
    int in_idx = d * slice_size + y * width + x;
    ...
    // Calculate transposed position within the same depth slice
    x = blockIdx.y * blockDim.y + threadIdx.x;
    y = blockIdx.x * blockDim.x + threadIdx.y;
    
    // Write transposed data, maintaining depth position
    if (x < height && y < width) {
        int out_idx = d * slice_size + y * height + x;
...
{   
    /*--------------------------- Malloc and Memcpy ---------------------------*/
    float *d_Q, *d_K, *d_V, *d_O, *d_Kt, *d_Vt, *d_Ot;
    ...
    cudaMalloc(&d_Kt, tensor_size);
    cudaMalloc(&d_Vt, tensor_size);
    cudaMalloc(&d_Ot, tensor_size);
    ...
    /*--------------------------- Transpose for better memory coalesce ---------------------------*/
    matrix3D_inner_transpose<<<dim3((d+31)/32, (N+31)/32, B), dim3(32,32)>>>(d_K, d_Kt, B, N, d);
    matrix3D_inner_transpose<<<dim3((d+31)/32, (N+31)/32, B), dim3(32,32)>>>(d_V, d_Vt, B, N, d);
    ...
    flash_attention_kernel<<<grid_dim, block_dim, sram_size>>>
        (d_Q, d_Kt, d_Vt, d_Ot, Bc, Br, Tc, Tr, softmax_scale);

    /*--------------------------- Transpose back Ot ---------------------------*/
    matrix3D_inner_transpose<<<dim3((d+31)/32, (N+31)/32, B), dim3(32,32)>>>(d_Ot, d_O, B, N, d);
    cudaDeviceSynchronize();
