// Use cudaMemcpy instead of cudaMemcpyPeerAsync
...
void block_FW()
{
    ...
    for (int device_no = 0; device_no < num_gpus; device_no++)
    {
        ...
        cudaStreamCreate(&streams[device_no]);
    ...
    for (int r = 0; r < round; ++r)
    {
        ...
        for (int device_no = 0; device_no < num_gpus; device_no++)
        {
            ...
            phase2_kernel_col<<<dim3(round, 1), block>>>
                (d_Dist[device_no], n, r);
    ...
    cudaSetDevice(0);
    /*----------------------------Test 1: success----------------------------*/
    // cudaMemcpy(&h_Dist[0], d_Dist[0],
    //             n*n*sizeof(int), cudaMemcpyDeviceToHost);

    /*----------------------------Test 2: fail----------------------------*/
    // cudaMemcpyPeerAsync(d_Dist[1] + blocks_per_gpu*FW_BZ*n, 1,
    //                     d_Dist[0] + blocks_per_gpu*FW_BZ*n, 0,
    //                     (n-blocks_per_gpu)*FW_BZ*n*sizeof(int), streams[0]);
    ...
    //            (n-blocks_per_gpu)*FW_BZ*n*sizeof(int), cudaMemcpyDeviceToHost);

    /*----------------------------Test 3: success----------------------------*/
    // cudaMemcpyPeerAsync(d_Dist[1], 1,
    //                     d_Dist[0], 0,
    //                     n*n*sizeof(int), streams[0]);
    // cudaStreamSynchronize(streams[0]);
    // cudaSetDevice(1);
    // cudaMemcpy(h_Dist, d_Dist[1],
    //            n*n*sizeof(int), cudaMemcpyDeviceToHost);

    /*----------------------------Test 4: success----------------------------*/
    // cudaMemcpyPeerAsync(d_Dist[1] + 100, 1,
    //                     d_Dist[0] + 100, 0,
    //                     (n*n-100)*sizeof(int), streams[0]);
    // cudaStreamSynchronize(streams[0]);

    // cudaMemcpy(h_Dist, d_Dist[0],
    //            100*sizeof(int), cudaMemcpyDeviceToHost);
    // cudaSetDevice(1);
    // cudaMemcpy(h_Dist + 100, d_Dist[1] + 100,
    //            (n*n-100)*sizeof(int), cudaMemcpyDeviceToHost);

    /*----------------------------Test 5: success----------------------------*/
    cudaMemcpyPeerAsync(d_Dist[1] + blocks_per_gpu*FW_BZ*n, 1,
                        d_Dist[0] + blocks_per_gpu*FW_BZ*n, 0,
                        (round-blocks_per_gpu)*FW_BZ*n*sizeof(int), streams[0]);
    cudaStreamSynchronize(streams[0]);
    ...
    /*----------------------------Clean up----------------------------*/
    for (int device_no = 0; device_no < num_gpus; device_no++) {
        cudaStreamDestroy(streams[device_no]);
