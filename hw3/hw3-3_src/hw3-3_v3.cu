...
// Use cudaMemcpyPeerAsync instead of cudaMemcpy(DeviceToDevice)
// c06.1 -> 40.35 s
// c07.1 -> 61.92 s
...
void block_FW()
{
    ...
    for (int r = 0; r < round; ++r)
    {
        ...
        if (r < blocks_per_gpu) {
            // For the first half, transfer from gpu0 to gpu1 since pivot block row is in gpu0
            cudaMemcpyPeerAsync(d_Dist[1] + pivot_offset, 1,
                                d_Dist[0] + pivot_offset, 0,
						        n*FW_BZ*sizeof(int), streams[0]);
            cudaStreamSynchronize(streams[0]);
        } else {
            // For the second half, transfer from gpu1 to gpu0 since pivot block row is in gpu1
            cudaMemcpyPeerAsync(d_Dist[0] + pivot_offset, 0,
                                d_Dist[1] + pivot_offset, 1,
						        n*FW_BZ*sizeof(int), streams[1]);
